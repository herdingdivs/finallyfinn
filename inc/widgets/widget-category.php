    ...
    function widget($args, $instance){
        ...
        .cat_widgs_conts {		 		  
		 ...
		 .cat_widgs_conts h3{
		   ...
		   margin-top: 0;
           line-height: 15px;
           margin-bottom: 8px !important;		   font-weight: 700;
		 }
...
var height_canvas = jQuery('.widget_sauron__categimages').offset().top - 750;
					...
					<div class="flipper">
					  ...
					  <div class="cat_widgs">
							  ...
							  <div class="cat_widgs-img">
								 <?php if(($image_thumb_width>220)&&($image_thumb_height>150)){ ?>
								<div class="image_divs" style="background: url(<?php echo $background_image; ?>) no-repeat center !important; background-size:cover !important; "></div>
							<?php }
							else { ?>
								<div class="image_divs" style="background: url(<?php echo $background_image; ?>) no-repeat center !important;"></div>
							<?php }  ?>
							  </div>
					  </div>
  ...
  /*Creates the form for the widget in the back-end. */
    function form($instance){
		...
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php echo __("Title:", "sauron"); ?></label><input class="widefat" id="<?php echo  $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
		
		<p><label for="<?php echo $this->get_field_id('description'); ?>"></label><textarea class="widefat" id="<?php echo  $this->get_field_id('description'); ?>" name="<?php echo $this->get_field_name('description'); ?>"><?php echo $description; ?></textarea></p>
		
		<p>
			<label for="<?php echo $this->get_field_id('text_info'); ?>"><?php echo __("Post text information backgroung color:", "sauron"); ?></label>
			<input id="<?php echo $this->get_field_id('text_info'); ?>" class="color wdwt_cat_color_input wp-color-picker"  value="<?php echo $instance['text_info'];?>" name="<?php echo  $this->get_field_name('text_info'); ?>" ></input>
		</p>
		
		<p><label for="<?php echo $this->get_field_id('categ_id'); ?>"><?php echo __("Select Category:", "sauron"); ?></label>
     ...
     <?php }
		...
		<p><label for="<?php echo $this->get_field_id('post_count'); ?>"><?php echo __("Number of Posts:", "sauron"); ?></label><input id="<?php echo  $this->get_field_id('post_count'); ?>" name="<?php echo $this->get_field_name('post_count'); ?>" type="text" value="<?php echo $post_count; ?>" size="6"/></p>
<?php		
}
